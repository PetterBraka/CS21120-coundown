...
/* All data is stored here so that it doesn't have
to be passed down during recursion - saves time */
...
/* Figure out which part of the numbers array we're
going to be addressing */
nbase = base + no + 1;
/* Our last calculation came up with a solution, so output the
path so far and stop */
...
//    System.out.println("operations: "+operations);
    ...
    {
    /* We're at the bottom of the recursion and haven't
    found a solution, so do nothing */
    }
else
    {
    /* We're at a higher level than the bottom of the
    recursion, and haven't found a solution, so iterate
    through the  possible number pairs and operations */
    for (a = 1;a < no;++a)
        {
        for (b = 1;b < no;++b)
            {
            if (a > b)
                ...
                addition will be done with this number pair ordered the
                other way around, and will give the same result */
